...
def generate_token(detail:generate_token_schema, db: Session = Depends(get_db)):
    ...
    try:
        ...
        theatre_auth_model = db.query(models.theatre_auth_model).filter(models.theatre_auth_model.theatre_id == detail.theatre_id).first()

        if theatre_auth_model is None:
            return JSONResponse(content={
                "error":"Invlalid Theatre",
                "status":999
            }, status_code=400)


        if detail.api_username == theatre_auth_model.api_username and detail.api_password == theatre_auth_model.api_password and detail.api_secret == theatre_auth_model.api_secret:
