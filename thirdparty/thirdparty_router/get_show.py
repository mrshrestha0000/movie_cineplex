...
def get_movie(details : theatre_show, db: Session = Depends(get_db), authenticate =Depends(auth.thirdparty_auth)):
    # theatre_info_model = models.theatre_info_model.all()
    show_model = db.query(models.show_model).filter(models.show_model.theatre_id == details.theatre_id and models.show_model.movie_id == details.movie_id)
