...
def seat_list(details, db):
    seat_detail_model = (
        ...
        .filter(
            and_(
                models.seat_detail_model.theatre_id == details["theatre_id"],
                models.seat_detail_model.audi_id == details["audi_id"],
            )
        )
        .all()
    )
...
def get_seat(
    ...
    return JSONResponse(content={"status": 000, "data": list_of_seat}, status_code=200)
