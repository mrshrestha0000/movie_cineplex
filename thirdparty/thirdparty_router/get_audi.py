...
def get_theatre_list(details : theatre_audi, db: Session = Depends(get_db), authenticate =Depends(auth.thirdparty_auth)):
    # theatre_info_model = models.theatre_info_model.all()
    theatre_audi_model = db.query(models.theatre_audi_model).filter(models.theatre_audi_model.theatre_id == details.theatre_id)
    list_audi = []
