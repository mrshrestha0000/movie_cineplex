...
def get_movie(
    details: theatre_movie,
    db: Session = Depends(get_db),
    authenticate=Depends(auth.thirdparty_auth),
):
    movie_model = db.query(models.movie_model).filter(
        models.movie_model.theatre_id == details.theatre_id
    )
    list_movie = []
    ...
    if movie_model is None:
        ...
        return JSONResponse(
            content={"status": 000, "data": list_movie}, status_code=200
        )
